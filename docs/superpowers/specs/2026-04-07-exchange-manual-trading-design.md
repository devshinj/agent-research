...
**중단 상단 — 캔들차트:**
- `lightweight-charts` 사용 (Dashboard와 동일 라이브러리)
- 캔들스틱 차트 + 하단 거래량 히스토그램
- 타임프레임 전환 버튼 그룹: 1분 / 5분 / 15분 / 1시간 / 4시간 / 일봉
- 데이터 소스: `/api/dashboard/candles` (timeframe 파라미터 추가)
- 실시간 업데이트: WebSocket 티커로 현재 캔들 실시간 갱신 (close/high/low), 새 캔들 시작 시 자동 추가
- 인터랙션: 크로스헤어 + OHLCV 툴팁, 스크롤 줌, 드래그 패닝
- 포지션 오버레이 (보유 중일 때): 평균 매입가 수평 점선 (accent), 예약 손절가 수평선 (빨강), 예약 익절가 수평선 (초록)
