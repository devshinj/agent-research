...
class PortfolioRepository:
    ...
    async def save_account(self, account: PaperAccount) -> None:
        now = int(time.time())
        await self._db.conn.execute(
            """INSERT INTO account_state (id, cash_balance, updated_at)
               VALUES (1, ?, ?)
               ON CONFLICT(id) DO UPDATE SET
                 cash_balance=excluded.cash_balance,
                 updated_at=excluded.updated_at""",
            (str(account.cash_balance), now),
        )
        await self._db.conn.execute("DELETE FROM positions")
    ...
    async def load_account(self, initial_balance: Decimal) -> PaperAccount | None:
        cursor = await self._db.conn.execute(
            "SELECT cash_balance FROM account_state WHERE id=1"
        )
        row = await cursor.fetchone()
        if not row:
            return None
    ...
    async def save_risk_state(self, state: dict[str, object]) -> None:
        ...
        await self._db.conn.execute(
            """INSERT INTO risk_state
               (id, consecutive_losses, cooldown_until, daily_loss, daily_trades, current_day, updated_at)
               VALUES (1, ?, ?, ?, ?, ?, ?)
               ON CONFLICT(id) DO UPDATE SET
    ...
    async def load_risk_state(self) -> dict[str, object] | None:
        cursor = await self._db.conn.execute(
            "SELECT consecutive_losses, cooldown_until, daily_loss, daily_trades, current_day FROM risk_state WHERE id=1"
        )
        row = await cursor.fetchone()
        if not row:
            return None
